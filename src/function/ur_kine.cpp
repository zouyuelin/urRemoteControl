...
namespace ur_Kinematics {
  ...
  namespace {
    ...
    //#define UR5_PARAMS
    #ifdef UR5_PARAMS
    const double d1 =  0.08923088135691268;
    const double a2 = -0.4255707701926842;
    const double a3 = -0.3921366434138572;
    const double d4 =  0.1107209342401144;
    const double d5 =  0.0949537146883094;
    const double d6 =  0.08254762185089397;
    #endif

    //#define UR3_PARAMS
    #ifdef UR3_PARAMS
    const double d1 =  0.152141420;
    const double a2 = -0.2438988106;
    const double a3 = -0.2134195661;
    const double d4 =  0.130774288;
    const double d5 =  0.085426883;
    const double d6 =  0.092375996;
    #endif
  }
  ...
  void jacobian(const double* q, double* J, double dx , double dy , double dz){
    double d6_ = d6 + dz;
    double s1 = sin(*q), c1 = cos(*q); q++;
    double q23 = *q, q234 = *q, s2 = sin(*q), c2 = cos(*q); q++;
    double s3 = sin(*q), c3 = cos(*q); q23 += *q; q234 += *q; q++;
    ...
    double c2345 = cos(q2345), c234_5 = cos(q234_5);

    *J = (c1*c5 + c234*s1*s5)*d6_ + dx*(c1*s5 - c234*c5*s1) + d4*c1 - a2*c2*s1 - d5*s234*s1 + dy*s234*s1 - a3*c2*c3*s1 + a3*s1*s2*s3;J++;
    *J = s234*c1*s5*d6_ - c1*(a3*s23 + a2*s2 - d5*c234 + dy*c234 + dx*s234*c5); J++;
    *J = s234*c1*s5*d6_ - c1*(a3*s23 - d5*c234 + dy*c234 + dx*s234*c5); J++;
    *J = s234*c1*s5*d6_ - c1*(dy*c234 - d5*c234 + dx*s234*c5); J++;
    *J = (c1*c2*c5*s3*s4 - c1*c2*c3*c4*c5 - s1*s5 + c1*c3*c5*s2*s4 + c1*c4*c5*s2*s3)*d6_
            + dx*c5*s1 + dx*c1*c2*s3*s4*s5 + dx*c1*c3*s2*s4*s5 + dx*c1*c4*s2*s3*s5 - dx*c1*c2*c3*c4*s5; J++;
    *J = dx*c1*s2*s3*s4 - dx*c1*c2*c3*s4 - dx*c1*c2*c4*s3 - dx*c1*c3*c4*s2 - dy*s1*s5
            + dy*c1*c2*c5*s3*s4 + dy*c1*c3*c5*s2*s4 + dy*c1*c4*c5*s2*s3 - dy*c1*c2*c3*c4*c5; J++;

    *J = (c5*s1 - c234*c1*s5)*d6_ + dx*(s1*s5 + c234*c1*c5) + d4*s1 + a2*c1*c2 + d5*s234*c1
            - dy*s234*c1 + a3*c1*c2*c3 - a3*c1*s2*s3;J++;
    *J = s234*s1*s5*d6_ - s1*(a3*s23 + a2*s2 - d5*c234 + dy*c234 + dx*s234*c5); J++;
    *J = s234*s1*s5*d6_ - s1*(a3*s23 - d5*c234 + dy*c234 + dx*s234*c5); J++;
    *J = s234*s1*s5*d6_ - s1*(dy*c234 - d5*c234 + dx*s234*c5); J++;
    *J = (c1*s5 - c2*c3*c4*c5*s1 + c2*c5*s1*s3*s4 + c3*c5*s1*s2*s4 + c4*c5*s1*s2*s3)*d6_
            + dx*c2*s1*s3*s4*s5 - dx*c2*c3*c4*s1*s5 - dx*c1*c5 + dx*c3*s1*s2*s4*s5 + dx*c4*s1*s2*s3*s5; J++;
    *J = dy*c1*s5 - dx*c2*c3*s1*s4 - dx*c2*c4*s1*s3 - dx*c3*c4*s1*s2 + dx*s1*s2*s3*s4
            + dy*c2*c5*s1*s3*s4 + dy*c3*c5*s1*s2*s4 + dy*c4*c5*s1*s2*s3 - dy*c2*c3*c4*c5*s1; J++;

    *J = 0;J++;
    *J = (s234_5/2 - s2345/2)*d6_ + (dx*c2345)/2 + a3*c23 + a2*c2 + (dx*c234_5)/2 + d5*s234 - dy*s234; J++;
    *J = (s234_5/2 - s2345/2)*d6_ + (dx*c2345)/2 + a3*c23 + (dx*c234_5)/2 + d5*s234 - dy*s234; J++;
    *J = (s234_5/2 - s2345/2)*d6_ + (dx*c2345)/2 + (dx*c234_5)/2 + d5*s234 - dy*s234; J++;
    *J = (-s234*c5)*d6_ - dx*s234*s5; J++;
    *J = dx*c234 - (dy*s234_5)/2 - (dy*s2345)/2; J++;
  ...
  void inverseK(const double* T, double *sq, double* q_sols, double q6_des, bool vetex){
    //    kinematic inverse calculation, example of usage:
    //    double q_sols[8*6];
    //    inverse(T,q,q_sols);
    ...
    for(int i = 0; i<6; i++) q_sols[i] = q_sols[minIndex*6+i];
  }
};
